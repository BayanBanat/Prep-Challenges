...
const cvFormatter = (arr) => {
    ...
    if(arr[i].yearsOfExperience>1){
      ...
      array.push({fullName:fullName,tech:arr[i].tech});
    }
    }
...
const applicationsStatics = (arr) => {
    // write your code here
    function Applicant(python_devs,javaScript_devs,dotNet_devs, java_devs,totalApplicants,rejectedApplicants){
...
Applicant.prototype.rejectedApplicants=function(){
    if(arr.names==null||arr.names==""&&arr.yearsOfExperience<=1){
...
const classesAvg = (data) => {
    ...
    for(let i=0;i<data.grades.length;i++){
        ...
        for(let j=0;j<grade.classes.length;j++){
            const closs=grade.classes[j];

        for(let m=0;m<closs.classScores.length;m++){
            sum+=closs.classScores[m];
        }
        const average=sum/closs.classScores.length;
        closs.average=average;
    }
    }
    // write your code here
};
